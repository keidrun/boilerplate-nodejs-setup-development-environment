...
Function removeDNS () {
    Get-Content $HostLocation | Where-Object {$_ -notmatch 'web'} | Set-Content $HostLocation
}
...
If($Args[0] -eq "setup") {
    ...
    init;
    docker exec -it $(docker-compose -f $COMPOSE_FILE ps -q web) /bin/bash -c "cd /srv;/bin/bash"
} ElseIf($Args[0] -eq "restart") {
    init;
    docker exec -it $(docker-compose -f $COMPOSE_FILE ps -q web) /bin/bash -c "cd /srv;forever restartall;"
} Else {
